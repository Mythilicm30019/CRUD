...
$CementGradeTypeData = "SELECT * FROM cement_grade_type ORDER BY cemtypeid ASC";
...
if (isset($_POST['btn_delete']) && $_POST['btn_delete'] === "Delete") {
	$id = intval($_POST['id']);
    $query = "UPDATE cement_grade_type SET active = 0 WHERE cemtypeid = $id"; // soft delete

    if (pg_query($query)) {
        $msg="Cement Grade Type Deleted Successfully";
		echo $msg;
    } else {
        $msg="Cement Grade Type not deleted, please try again...!!";
    }
...
if (isset($_POST['btn_undodelete']) && $_POST['btn_undodelete'] === "UndoDelete") {
	$id = intval($_POST['id']);
    $query = "UPDATE cement_grade_type SET active = 1 WHERE cemtypeid = $id"; // soft delete
    if (pg_query($query)) {
        $msg="Cement Grade Type Activated Successfully";
		echo $msg;
    } else {
        $msg="Cement Grade Type not activated, please try again...!!";
    }
    exit; 
}
...
<script type="text/javascript">
	...
	function goBack(){
		url = "CementGradeType.php";
		window.location.replace(url);
	}
	 function RedirectToEdit(EditId) {
        var url = "CementGradeType.php?action=EDIT&id=" + EditId;
        ...
        <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post" enctype="multipart/form-data" name="form" id="form1">
            ...
            <!--==============================Content=================================-->
			<div class="content">
				<div class="title">Cement Grade View & Edit</div>
									...
									<div class="div12">
										<div class="card cabox">
											<div class="face-static">
												<div class="card-header inkblue-card" align="center">&nbsp;Cement Grade Type - View</div>
                                                            	...
                                                            	<table id="example" class="display rtable mgtb-8" style="width:100%">
                                                                    ...
                                                                    <tbody>
								                                		...
								                                		<tr>
								                                			...
								                                			<td align="left"><?php echo $value['cement_type_desc'] ?></td>
								                                			<td align="left"><?php echo $value['cement_type_code'] ?></td>
                                                                            ...
                                                                            if($value['active'] == 1){ ?>
								                                				<td align="center" style="width:100px">
                                                                                    <!-- <a data-url="CementGradeType?id=<?php echo $value['cemtypeid']; ?>" class=" BtnHref btn btn-info" name="btn_edit" id="btn_edit">
																						Edit
																					</a> -->
																					<a  onclick="RedirectToEdit(<?php echo $value['cemtypeid']; ?>);" class=" BtnHref btn btn-info" name="btn_edit" id="btn_edit">
																						Edit
																					</a>
                                                                            <?php }
                                                                            else{ ?>								                                				
								                                			...
								                                			<td align="center" style="width:100px">
                                                                                <?php if($value['active'] == 1){ ?>
								                                					<button type="button" name = "btn_delete" id = "btn_delete" class="btn fa-btn-d gDelete tdelbtn Delete" data-id="<?php echo $value['cemtypeid']; ?>" title="Click here to delete" style="cursor: pointer;"><i class="fa fa-trash-o"></i></button>								
								                                				<?php }else{ ?>
								                                			...
								                                			<td align="center" style="width:100px">
				                                                                <?php if($value['active'] == 0){ ?>   
								                                					<button type="button" name = "btn_undo_delete" id = "btn_undo_delete" class="btn btn-default tdelbtn UndoDelete" data-id="<?php echo $value['cemtypeid']; ?>" title="Click here to activate" onClick="UndoDeleteMaterialSubCat();" style="cursor: pointer;"><i class="fa fa-recycle"></i></button>
								                                				<?php }else{ ?>
							...
							BootstrapDialog.show({
								...
								buttons: [{
									...
									action: function(dialog) {
										dialog.close();
										window.location.replace('CementGradeTypeView.php');
									}
								}]
							});
						}
