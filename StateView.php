...
$CementGradeTypeData = "SELECT * FROM state_master ORDER BY state_id ASC";
...
if (isset($_POST['btn_delete']) && $_POST['btn_delete'] === "Delete") {
	$id = intval($_POST['id']);
    $query = "UPDATE state_master SET active = 0 WHERE state_id = $id"; // soft delete

    if (pg_query($query)) {
        $msg="State Master Deleted Successfully";
		echo $msg;
    } else {
        $msg="State Master not deleted, please try again...!!";
    }
...
if (isset($_POST['btn_undodelete']) && $_POST['btn_undodelete'] === "UndoDelete") {
	$id = intval($_POST['id']);
    $query = "UPDATE state_master SET active = 1 WHERE state_id = $id"; // soft delete
    if (pg_query($query)) {
...
<script type="text/javascript">
	...
	function goBack(){
	 ...
	 function RedirectToEdit(EditId) {
        var url = "StateMaster.php?action=EDIT&id=" + EditId;
        ...
        <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post" enctype="multipart/form-data" name="form" id="form1">
            ...
            <!--==============================Content=================================-->
			<div class="content">
				<div class="title">State View & Edit</div>
									...
									<div class="div12">
										<div class="card cabox">
											<div class="face-static">
												<div class="card-header inkblue-card" align="center">&nbsp;State - View</div>
                                                            	...
                                                            	<table id="example" class="display rtable mgtb-8" style="width:100%">
                                                                    ...
                                                                    <tbody>
								                                		...
								                                		<tr>
								                                			...
								                                			<td align="left"><?php echo $value['state_description'] ?></td>
								                                			<td align="left"><?php echo $value['state_code'] ?></td>
 ...
 -->                                                                            <?php if($value['active'] == 1){ ?>
                                                                            ...
                                                                            if($value['active'] == 1){ ?>
								                                				<td align="center" style="width:100px">
                                                                                    <!-- <a data-url="CementGradeType?id=<?php echo $value['state_id']; ?>" class=" BtnHref btn btn-info" name="btn_edit" id="btn_edit">
																						Edit
																					</a> -->
																					<a  onclick="RedirectToEdit(<?php echo $value['state_id']; ?>);" class=" BtnHref btn btn-info" name="btn_edit" id="btn_edit">
																						Edit
																					</a>
                                                                            <?php }
                                                                            else{ ?>								                                				
								                                			...
								                                			<td align="center" style="width:100px">
                                                                                <?php if($value['active'] == 1){ ?>
								                                					<button type="button" name = "btn_delete" id = "btn_delete" class="btn fa-btn-d gDelete tdelbtn Delete" data-id="<?php echo $value['state_id']; ?>" title="Click here to delete" style="cursor: pointer;"><i class="fa fa-trash-o"></i></button>								
								                                				<?php }else{ ?>
								                                			...
								                                			<td align="center" style="width:100px">
				                                                                <?php if($value['active'] == 0){ ?>   
								                                					<button type="button" name = "btn_undo_delete" id = "btn_undo_delete" class="btn btn-default tdelbtn UndoDelete" data-id="<?php echo $value['state_id']; ?>" title="Click here to activate" onClick="UndoDeleteMaterialSubCat();" style="cursor: pointer;"><i class="fa fa-recycle"></i></button>
								                                				<?php }else{ ?>
    		            ...
    		            if(response != ""){
							BootstrapDialog.show({
								...
								buttons: [{
									...
									action: function(dialog) {
										dialog.close();
										window.location.replace('StateView.php');
									}
								}]
							});
						}
