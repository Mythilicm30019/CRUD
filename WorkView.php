...
$CementGradeTypeData = "SELECT * FROM work_detail ORDER BY  work_id ASC";
...
if (isset($_POST['btn_delete']) && $_POST['btn_delete'] === "Delete") {
	$id = intval($_POST['id']);
    $query = "UPDATE work_detail SET active = 0 WHERE  work_id = $id"; // soft delete

    if (pg_query($query)) {
        $msg="Work Detail  Type Deleted Successfully";
		echo $msg;
    } else {
        $msg="Work Detail  Type not deleted, please try again...!!";
    }
...
if (isset($_POST['btn_undodelete']) && $_POST['btn_undodelete'] === "UndoDelete") {
	$id = intval($_POST['id']);
    $query = "UPDATE work_detail SET active = 1 WHERE  work_id = $id"; // soft delete
    if (pg_query($query)) {
        $msg="Workers Type Activated Successfully";
		echo $msg;
    } else {
        $msg="Workers Grade Type not activated, please try again...!!";
    }
    exit; 
}
...
<script type="text/javascript">
	...
	function goBack(){
	 ...
	 function RedirectToEdit(EditId) {
        var url = "WorkMaster.php?action=EDIT&id=" + EditId;
        ...
        <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post" enctype="multipart/form-data" name="form" id="form1">
            ...
            <!--==============================Content=================================-->
			<div class="content">
				<div class="title">Work  View & Edit</div>
									...
									<div class="div12">
										<div class="card cabox">
											<div class="face-static">
												<div class="card-header inkblue-card" align="center">&nbsp;Work Master Type - View</div>
 ...
 -->									                                		<th class="colhead" style="text-align:center">Status</th>
                                                                    ...
                                                                    <tbody>
								                                		...
								                                		<tr>
								                                			...
								                                			<td align="left"><?php echo $value['work_code'] ?></td>
 ...
 -->                                                                            <?php if($value['active'] == 1){ ?>
                                                                            ...
                                                                            if($value['active'] == 1){ ?>
								                                				<td align="center" style="width:100px">
                                                                                    <!-- <a data-url="CementGradeType?id=<?php echo $value['work_id']; ?>" class=" BtnHref btn btn-info" name="btn_edit" id="btn_edit">
																						Edit
																					</a> -->
																					<a  onclick="RedirectToEdit(<?php echo $value['work_id']; ?>);" class=" BtnHref btn btn-info" name="btn_edit" id="btn_edit">
																						Edit
																					</a>
                                                                            <?php }
                                                                            else{ ?>								                                				
								                                			...
								                                			<td align="center" style="width:100px">
                                                                                <?php if($value['active'] == 1){ ?>
								                                					<button type="button" name = "btn_delete" id = "btn_delete" class="btn fa-btn-d gDelete tdelbtn Delete" data-id="<?php echo $value['work_id']; ?>" title="Click here to delete" style="cursor: pointer;"><i class="fa fa-trash-o"></i></button>								
								                                				<?php }else{ ?>
								                                			...
								                                			<td align="center" style="width:100px">
				                                                                <?php if($value['active'] == 0){ ?>   
								                                					<button type="button" name = "btn_undo_delete" id = "btn_undo_delete" class="btn btn-default tdelbtn UndoDelete" data-id="<?php echo $value['work_id']; ?>" title="Click here to activate" onClick="UndoDeleteMaterialSubCat();" style="cursor: pointer;"><i class="fa fa-recycle"></i></button>
								                                				<?php }else{ ?>
							...
							BootstrapDialog.show({
								...
								buttons: [{
									...
									action: function(dialog) {
										dialog.close();
										window.location.replace('WorkView.php');
									}
								}]
							});
						}
