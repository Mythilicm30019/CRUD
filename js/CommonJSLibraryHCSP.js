...
function FindTotalAmountNewMat(){ 
	var TotalAmt = 0; var TransAmt = 0; var PspAmt = 0; var TotalAmtMat = 0;
	...
	$('#txt_sub_total_amount').val(TotalAmt);
	var TransDesc 	= $("#txt_trans_desc").val();
	var TransPerc 	= $("#txt_trans_perc").val();
	var PspDesc 	= $("#txt_psp_desc").val();
	var PspPerc 	= $("#txt_psp_perc").val(); 
	var TransAmt 	= Number(TotalAmt) * Number(TransPerc) / 100;
	var PspAmt 		= Number(TotalAmt) * Number(PspPerc) / 100;
	if(TransAmt > 0){
	...
	if(PspAmt > 0){
		PspAmt = PspAmt.toFixed(2);
	}else{
		PspAmt = 0.00;
	}
	$("#txt_trans_amount").val(TransAmt);
	$("#txt_psp_amount").val(PspAmt);
	TotalAmtMat = Number(TotalAmt) + Number(TransAmt) + Number(PspAmt);
	if(TotalAmtMat >0){
		TotalAmtMat = TotalAmtMat.toFixed(2); 
	}
	$("#txt_mat_total_amount").val(TotalAmtMat);
}
...
$("#top").submit(function(event){
	if(KillEvent == 0){
		var Level = 0; var CurrLevel = ""; var GrError = 0; var GrTemp = 0; var ChGr = 0; var ChildId = ""; var ParentId = "";
